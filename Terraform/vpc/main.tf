...
resource "aws_internet_gateway_bitwala" "main" {
...
resource "aws_nat_gateway_bitwala" "main" {
  count         = length(var.private_subnets)
  allocation_id = element(aws_eip.nat.*.id, count.index)
  subnet_id     = element(aws_subnet.public.*.id, count.index)
  depends_on    = [aws_internet_gateway_bitwala.main]
...
resource "aws_route" "private" {
  ...
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = element(aws_nat_gateway_bitwala.main.*.id, count.index)
}
